...
#cmt_local_tagfile = $(ExtractRecInfo_tag).make
cmt_local_tagfile = $(bin)$(ExtractRecInfo_tag).make
...
cmt_local_setup_ExtractRecInfo = $(bin)setup_ExtractRecInfo$$$$.make
...
ExtractRecInfo_extratags = -tag_add=target_ExtractRecInfo

#$(cmt_local_tagfile_ExtractRecInfo) : $(cmt_lock_setup)
ifndef QUICK
$(cmt_local_tagfile_ExtractRecInfo) ::
else
$(cmt_local_tagfile_ExtractRecInfo) :
endif
	$(echo) "(constituents.make) Rebuilding $@"; \
	  if test -f $(cmt_local_tagfile_ExtractRecInfo); then /bin/rm -f $(cmt_local_tagfile_ExtractRecInfo); fi ; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_extratags) build tag_makefile >>$(cmt_local_tagfile_ExtractRecInfo)
	$(echo) "(constituents.make) Rebuilding $(cmt_final_setup_ExtractRecInfo)"; \
	  test ! -f $(cmt_local_setup_ExtractRecInfo) || \rm -f $(cmt_local_setup_ExtractRecInfo); \
	  trap '\rm -f $(cmt_local_setup_ExtractRecInfo)' 0 1 2 15; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_extratags) show setup >$(cmt_local_setup_ExtractRecInfo) && \
	  if [ -f $(cmt_final_setup_ExtractRecInfo) ] && \
	    \cmp -s $(cmt_final_setup_ExtractRecInfo) $(cmt_local_setup_ExtractRecInfo); then \
	    \rm $(cmt_local_setup_ExtractRecInfo); else \
	    \mv -f $(cmt_local_setup_ExtractRecInfo) $(cmt_final_setup_ExtractRecInfo); fi
...
not_ExtractRecInfo_dependencies = { n=0; for p in $?; do m=0; for d in $(ExtractRecInfo_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
ExtractRecInfodirs :
	@if test ! -d $(bin)ExtractRecInfo; then $(mkdir) -p $(bin)ExtractRecInfo; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)ExtractRecInfo
else
ExtractRecInfodirs : ;
endif
...
#	$(echo) "STRUCTURED_OUTPUT="$(bin)ExtractRecInfo
#
#$(cmt_local_ExtractRecInfo_makefile) :: $(ExtractRecInfo_dependencies) $(cmt_local_tagfile_ExtractRecInfo) build_library_links dirs ExtractRecInfodirs
#else
#$(cmt_local_ExtractRecInfo_makefile) :: $(ExtractRecInfo_dependencies) $(cmt_local_tagfile_ExtractRecInfo) build_library_links dirs
#endif
#else
#$(cmt_local_ExtractRecInfo_makefile) :: $(cmt_local_tagfile_ExtractRecInfo)
#endif

ifdef cmt_ExtractRecInfo_has_target_tag

ifndef QUICK
$(cmt_local_ExtractRecInfo_makefile) : $(ExtractRecInfo_dependencies) build_library_links
	$(echo) "(constituents.make) Building ExtractRecInfo.make"; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_extratags) build constituent_config -out=$(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo
else
$(cmt_local_ExtractRecInfo_makefile) : $(ExtractRecInfo_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_ExtractRecInfo) ] || \
	  [ ! -f $(cmt_final_setup_ExtractRecInfo) ] || \
	  $(not_ExtractRecInfo_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building ExtractRecInfo.make"; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_extratags) build constituent_config -out=$(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo; \
	  fi
endif
...
$(cmt_local_ExtractRecInfo_makefile) : $(ExtractRecInfo_dependencies) build_library_links
	$(echo) "(constituents.make) Building ExtractRecInfo.make"; \
	  $(cmtexe) -f=$(bin)ExtractRecInfo.in -tag=$(tags) $(ExtractRecInfo_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo
else
$(cmt_local_ExtractRecInfo_makefile) : $(ExtractRecInfo_dependencies) $(cmt_build_library_linksstamp) $(bin)ExtractRecInfo.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_ExtractRecInfo) ] || \
	  [ ! -f $(cmt_final_setup_ExtractRecInfo) ] || \
	  $(not_ExtractRecInfo_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_extratags) build constituent_makefile -out=$(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo

ExtractRecInfo :: $(ExtractRecInfo_dependencies) $(cmt_local_ExtractRecInfo_makefile) dirs ExtractRecInfodirs
	$(echo) "(constituents.make) Starting ExtractRecInfo"
	@if test -f $(cmt_local_ExtractRecInfo_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo; \
	  fi
#	@$(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) ExtractRecInfo
	$(echo) "(constituents.make) ExtractRecInfo done"

clean :: ExtractRecInfoclean

ExtractRecInfoclean :: $(ExtractRecInfoclean_dependencies) ##$(cmt_local_ExtractRecInfo_makefile)
	$(echo) "(constituents.make) Starting ExtractRecInfoclean"
	@-if test -f $(cmt_local_ExtractRecInfo_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) ExtractRecInfoclean; \
	fi
	$(echo) "(constituents.make) ExtractRecInfoclean done"
#	@-$(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) ExtractRecInfoclean

##	  /bin/rm -f $(cmt_local_ExtractRecInfo_makefile) $(bin)ExtractRecInfo_dependencies.make

install :: ExtractRecInfoinstall

ExtractRecInfoinstall :: $(ExtractRecInfo_dependencies) $(cmt_local_ExtractRecInfo_makefile)
	$(echo) "(constituents.make) Starting install ExtractRecInfo"
	@-$(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) install
	$(echo) "(constituents.make) install ExtractRecInfo done"

uninstall : ExtractRecInfouninstall

$(foreach d,$(ExtractRecInfo_dependencies),$(eval $(d)uninstall_dependencies += ExtractRecInfouninstall))

ExtractRecInfouninstall : $(ExtractRecInfouninstall_dependencies) ##$(cmt_local_ExtractRecInfo_makefile)
	$(echo) "(constituents.make) Starting uninstall ExtractRecInfo"
	@if test -f $(cmt_local_ExtractRecInfo_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_ExtractRecInfo_makefile) uninstall
	$(echo) "(constituents.make) uninstall ExtractRecInfo done"

remove_library_links :: ExtractRecInfouninstall
...
#cmt_local_tagfile_install_runtime = $(ExtractRecInfo_tag)_install_runtime.make
cmt_local_tagfile_install_runtime = $(bin)$(ExtractRecInfo_tag)_install_runtime.make
...
#cmt_final_setup_install_runtime = $(bin)ExtractRecInfo_install_runtimesetup.make
...
#cmt_local_tagfile_install_runtime = $(ExtractRecInfo_tag).make
cmt_local_tagfile_install_runtime = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_install_runtime = $(bin)setup.make
#cmt_final_setup_install_runtime = $(bin)ExtractRecInfosetup.make
cmt_local_install_runtime_makefile = $(bin)install_runtime.make
...
#cmt_local_tagfile_config = $(ExtractRecInfo_tag)_config.make
cmt_local_tagfile_config = $(bin)$(ExtractRecInfo_tag)_config.make
cmt_local_setup_config = $(bin)setup_config$$$$.make
cmt_final_setup_config = $(bin)setup_config.make
#cmt_final_setup_config = $(bin)ExtractRecInfo_configsetup.make
...
#cmt_local_tagfile_config = $(ExtractRecInfo_tag).make
cmt_local_tagfile_config = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_config = $(bin)setup.make
#cmt_final_setup_config = $(bin)ExtractRecInfosetup.make
cmt_local_config_makefile = $(bin)config.make
...
#cmt_local_tagfile_install_includes = $(ExtractRecInfo_tag)_install_includes.make
cmt_local_tagfile_install_includes = $(bin)$(ExtractRecInfo_tag)_install_includes.make
...
#cmt_final_setup_install_includes = $(bin)ExtractRecInfo_install_includessetup.make
...
#cmt_local_tagfile_install_includes = $(ExtractRecInfo_tag).make
cmt_local_tagfile_install_includes = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_install_includes = $(bin)setup.make
#cmt_final_setup_install_includes = $(bin)ExtractRecInfosetup.make
cmt_local_install_includes_makefile = $(bin)install_includes.make
...
#cmt_local_tagfile_make = $(ExtractRecInfo_tag)_make.make
cmt_local_tagfile_make = $(bin)$(ExtractRecInfo_tag)_make.make
cmt_local_setup_make = $(bin)setup_make$$$$.make
cmt_final_setup_make = $(bin)setup_make.make
#cmt_final_setup_make = $(bin)ExtractRecInfo_makesetup.make
...
#cmt_local_tagfile_make = $(ExtractRecInfo_tag).make
cmt_local_tagfile_make = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_make = $(bin)setup.make
#cmt_final_setup_make = $(bin)ExtractRecInfosetup.make
cmt_local_make_makefile = $(bin)make.make
...
#cmt_local_tagfile_CompilePython = $(ExtractRecInfo_tag)_CompilePython.make
cmt_local_tagfile_CompilePython = $(bin)$(ExtractRecInfo_tag)_CompilePython.make
cmt_local_setup_CompilePython = $(bin)setup_CompilePython$$$$.make
cmt_final_setup_CompilePython = $(bin)setup_CompilePython.make
#cmt_final_setup_CompilePython = $(bin)ExtractRecInfo_CompilePythonsetup.make
...
#cmt_local_tagfile_CompilePython = $(ExtractRecInfo_tag).make
cmt_local_tagfile_CompilePython = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_CompilePython = $(bin)setup.make
#cmt_final_setup_CompilePython = $(bin)ExtractRecInfosetup.make
cmt_local_CompilePython_makefile = $(bin)CompilePython.make
...
#cmt_local_tagfile_qmtest_run = $(ExtractRecInfo_tag)_qmtest_run.make
cmt_local_tagfile_qmtest_run = $(bin)$(ExtractRecInfo_tag)_qmtest_run.make
cmt_local_setup_qmtest_run = $(bin)setup_qmtest_run$$$$.make
cmt_final_setup_qmtest_run = $(bin)setup_qmtest_run.make
#cmt_final_setup_qmtest_run = $(bin)ExtractRecInfo_qmtest_runsetup.make
...
#cmt_local_tagfile_qmtest_run = $(ExtractRecInfo_tag).make
cmt_local_tagfile_qmtest_run = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_qmtest_run = $(bin)setup.make
#cmt_final_setup_qmtest_run = $(bin)ExtractRecInfosetup.make
cmt_local_qmtest_run_makefile = $(bin)qmtest_run.make
...
#cmt_local_tagfile_qmtest_summarize = $(ExtractRecInfo_tag)_qmtest_summarize.make
cmt_local_tagfile_qmtest_summarize = $(bin)$(ExtractRecInfo_tag)_qmtest_summarize.make
...
#cmt_final_setup_qmtest_summarize = $(bin)ExtractRecInfo_qmtest_summarizesetup.make
...
#cmt_local_tagfile_qmtest_summarize = $(ExtractRecInfo_tag).make
cmt_local_tagfile_qmtest_summarize = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_qmtest_summarize = $(bin)setup.make
#cmt_final_setup_qmtest_summarize = $(bin)ExtractRecInfosetup.make
cmt_local_qmtest_summarize_makefile = $(bin)qmtest_summarize.make
...
#cmt_local_tagfile_TestPackage = $(ExtractRecInfo_tag)_TestPackage.make
cmt_local_tagfile_TestPackage = $(bin)$(ExtractRecInfo_tag)_TestPackage.make
cmt_local_setup_TestPackage = $(bin)setup_TestPackage$$$$.make
cmt_final_setup_TestPackage = $(bin)setup_TestPackage.make
#cmt_final_setup_TestPackage = $(bin)ExtractRecInfo_TestPackagesetup.make
...
#cmt_local_tagfile_TestPackage = $(ExtractRecInfo_tag).make
cmt_local_tagfile_TestPackage = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_TestPackage = $(bin)setup.make
#cmt_final_setup_TestPackage = $(bin)ExtractRecInfosetup.make
cmt_local_TestPackage_makefile = $(bin)TestPackage.make
...
#cmt_local_tagfile_TestProject = $(ExtractRecInfo_tag)_TestProject.make
cmt_local_tagfile_TestProject = $(bin)$(ExtractRecInfo_tag)_TestProject.make
cmt_local_setup_TestProject = $(bin)setup_TestProject$$$$.make
cmt_final_setup_TestProject = $(bin)setup_TestProject.make
#cmt_final_setup_TestProject = $(bin)ExtractRecInfo_TestProjectsetup.make
...
#cmt_local_tagfile_TestProject = $(ExtractRecInfo_tag).make
cmt_local_tagfile_TestProject = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_TestProject = $(bin)setup.make
#cmt_final_setup_TestProject = $(bin)ExtractRecInfosetup.make
cmt_local_TestProject_makefile = $(bin)TestProject.make
...
#cmt_local_tagfile_new_rootsys = $(ExtractRecInfo_tag)_new_rootsys.make
cmt_local_tagfile_new_rootsys = $(bin)$(ExtractRecInfo_tag)_new_rootsys.make
cmt_local_setup_new_rootsys = $(bin)setup_new_rootsys$$$$.make
cmt_final_setup_new_rootsys = $(bin)setup_new_rootsys.make
#cmt_final_setup_new_rootsys = $(bin)ExtractRecInfo_new_rootsyssetup.make
...
#cmt_local_tagfile_new_rootsys = $(ExtractRecInfo_tag).make
cmt_local_tagfile_new_rootsys = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_new_rootsys = $(bin)setup.make
#cmt_final_setup_new_rootsys = $(bin)ExtractRecInfosetup.make
cmt_local_new_rootsys_makefile = $(bin)new_rootsys.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(ExtractRecInfo_tag)_DatabaseSvc_check_install_scripts.make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(ExtractRecInfo_tag)_DatabaseSvc_check_install_scripts.make
...
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)ExtractRecInfo_DatabaseSvc_check_install_scriptssetup.make
...
#cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(ExtractRecInfo_tag).make
cmt_local_tagfile_DatabaseSvc_check_install_scripts = $(bin)$(ExtractRecInfo_tag).make
cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)setup.make
#cmt_final_setup_DatabaseSvc_check_install_scripts = $(bin)ExtractRecInfosetup.make
cmt_local_DatabaseSvc_check_install_scripts_makefile = $(bin)DatabaseSvc_check_install_scripts.make
...
cmt_local_setup_ExtractRecInfo_check_install_runtime = $(bin)setup_ExtractRecInfo_check_install_runtime$$$$.make
...
ExtractRecInfo_check_install_runtime_extratags = -tag_add=target_ExtractRecInfo_check_install_runtime

#$(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) : $(cmt_lock_setup)
ifndef QUICK
$(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) ::
else
$(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) :
endif
	$(echo) "(constituents.make) Rebuilding $@"; \
	  if test -f $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime); then /bin/rm -f $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime); fi ; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) build tag_makefile >>$(cmt_local_tagfile_ExtractRecInfo_check_install_runtime)
	$(echo) "(constituents.make) Rebuilding $(cmt_final_setup_ExtractRecInfo_check_install_runtime)"; \
	  test ! -f $(cmt_local_setup_ExtractRecInfo_check_install_runtime) || \rm -f $(cmt_local_setup_ExtractRecInfo_check_install_runtime); \
	  trap '\rm -f $(cmt_local_setup_ExtractRecInfo_check_install_runtime)' 0 1 2 15; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) show setup >$(cmt_local_setup_ExtractRecInfo_check_install_runtime) && \
	  if [ -f $(cmt_final_setup_ExtractRecInfo_check_install_runtime) ] && \
	    \cmp -s $(cmt_final_setup_ExtractRecInfo_check_install_runtime) $(cmt_local_setup_ExtractRecInfo_check_install_runtime); then \
	    \rm $(cmt_local_setup_ExtractRecInfo_check_install_runtime); else \
	    \mv -f $(cmt_local_setup_ExtractRecInfo_check_install_runtime) $(cmt_final_setup_ExtractRecInfo_check_install_runtime); fi
...
not_ExtractRecInfo_check_install_runtime_dependencies = { n=0; for p in $?; do m=0; for d in $(ExtractRecInfo_check_install_runtime_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
ExtractRecInfo_check_install_runtimedirs :
	@if test ! -d $(bin)ExtractRecInfo_check_install_runtime; then $(mkdir) -p $(bin)ExtractRecInfo_check_install_runtime; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)ExtractRecInfo_check_install_runtime
else
ExtractRecInfo_check_install_runtimedirs : ;
endif
...
#	$(echo) "STRUCTURED_OUTPUT="$(bin)ExtractRecInfo_check_install_runtime
#
#$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) :: $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) build_library_links dirs ExtractRecInfo_check_install_runtimedirs
#else
#$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) :: $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) build_library_links dirs
#endif
#else
#$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) :: $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime)
#endif

ifdef cmt_ExtractRecInfo_check_install_runtime_has_target_tag

ifndef QUICK
$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) : $(ExtractRecInfo_check_install_runtime_dependencies) build_library_links
	$(echo) "(constituents.make) Building ExtractRecInfo_check_install_runtime.make"; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) build constituent_config -out=$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime
else
$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) : $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) ] || \
	  [ ! -f $(cmt_final_setup_ExtractRecInfo_check_install_runtime) ] || \
	  $(not_ExtractRecInfo_check_install_runtime_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building ExtractRecInfo_check_install_runtime.make"; \
	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) build constituent_config -out=$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime; \
	  fi
endif
...
$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) : $(ExtractRecInfo_check_install_runtime_dependencies) build_library_links
	$(echo) "(constituents.make) Building ExtractRecInfo_check_install_runtime.make"; \
	  $(cmtexe) -f=$(bin)ExtractRecInfo_check_install_runtime.in -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime
else
$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) : $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_build_library_linksstamp) $(bin)ExtractRecInfo_check_install_runtime.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_ExtractRecInfo_check_install_runtime) ] || \
	  [ ! -f $(cmt_final_setup_ExtractRecInfo_check_install_runtime) ] || \
	  $(not_ExtractRecInfo_check_install_runtime_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(ExtractRecInfo_check_install_runtime_extratags) build constituent_makefile -out=$(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime

ExtractRecInfo_check_install_runtime :: $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) dirs ExtractRecInfo_check_install_runtimedirs
	$(echo) "(constituents.make) Starting ExtractRecInfo_check_install_runtime"
	@if test -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime; \
	  fi
#	@$(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtime
	$(echo) "(constituents.make) ExtractRecInfo_check_install_runtime done"

clean :: ExtractRecInfo_check_install_runtimeclean

ExtractRecInfo_check_install_runtimeclean :: $(ExtractRecInfo_check_install_runtimeclean_dependencies) ##$(cmt_local_ExtractRecInfo_check_install_runtime_makefile)
	$(echo) "(constituents.make) Starting ExtractRecInfo_check_install_runtimeclean"
	@-if test -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtimeclean; \
	fi
	$(echo) "(constituents.make) ExtractRecInfo_check_install_runtimeclean done"
#	@-$(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) ExtractRecInfo_check_install_runtimeclean

##	  /bin/rm -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) $(bin)ExtractRecInfo_check_install_runtime_dependencies.make

install :: ExtractRecInfo_check_install_runtimeinstall

ExtractRecInfo_check_install_runtimeinstall :: $(ExtractRecInfo_check_install_runtime_dependencies) $(cmt_local_ExtractRecInfo_check_install_runtime_makefile)
	$(echo) "(constituents.make) Starting install ExtractRecInfo_check_install_runtime"
	@-$(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) install
	$(echo) "(constituents.make) install ExtractRecInfo_check_install_runtime done"

uninstall : ExtractRecInfo_check_install_runtimeuninstall

$(foreach d,$(ExtractRecInfo_check_install_runtime_dependencies),$(eval $(d)uninstall_dependencies += ExtractRecInfo_check_install_runtimeuninstall))

ExtractRecInfo_check_install_runtimeuninstall : $(ExtractRecInfo_check_install_runtimeuninstall_dependencies) ##$(cmt_local_ExtractRecInfo_check_install_runtime_makefile)
	$(echo) "(constituents.make) Starting uninstall ExtractRecInfo_check_install_runtime"
	@if test -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile); then \
	  $(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_ExtractRecInfo_check_install_runtime_makefile) uninstall
	$(echo) "(constituents.make) uninstall ExtractRecInfo_check_install_runtime done"

remove_library_links :: ExtractRecInfo_check_install_runtimeuninstall
