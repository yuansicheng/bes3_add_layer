...
//  double e = sqrt(ptrk*ptrk + mass * mass);
    double e = trk->energy();
    double ptrk = sqrt(e*e - mass * mass);
    double the = trk->theta();
    double phi = trk->phi();
    HepLorentzVector p4( ptrk* sin(the) * cos(phi), 
            ptrk * sin(the) * sin(phi),
...
void TrackPool::myAddMissTrack(const int number, const double mass, const HepLorentzVector p4) {
    //
    ...
    HepVector w(7,0);
    HepSymMatrix Ew(7,0);
    w[0] = p4.px();   // set a more reasonable value : Liang Liu
    w[1] = p4.py();
    w[2] = p4.pz();
    w[3] = sqrt(p4.px()*p4.px() + p4.py()*p4.py() + p4.pz()*p4.pz() + mass*mass);
    Ew[0][0] = 1E+6;
    Ew[1][1] = 1E+6;
    Ew[2][2] = 1E+6;
    wtrk.setW(w);
