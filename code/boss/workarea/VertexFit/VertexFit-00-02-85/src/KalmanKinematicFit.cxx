...
void printMat(HepMatrix mat, char *name){
...
void printMat(HepSymMatrix mat, char *name){
		cout << "Matrix : " << name << endl;
		for(int i  = 0; i < mat.num_row(); i++){
				for(int j  = 0; j < mat.num_col(); j++){
						cout  << setw(8) << mat[i][j] << "  ";
...
void initialMat(HepMatrix mat){
		for(int i  = 0; i < mat.num_col(); i++){
				for(int j  = 0; j < mat.num_row(); j++){
						mat[j][i]  = 0.0;
				}
		}
}

void initialMat(HepSymMatrix mat){
		for(int i  = 0; i < mat.num_col(); i++){
				for(int j  = 0; j < mat.num_row(); j++){
						mat[i][j]  = 0.0;
				}
...
//	printMat(m_A, "m_A_fit");
//	printMat(m_C0, "m_C0_fit");
...
//	initialMat(m_C0);
 //   HepSymMatrix m_C_1 = HepSymMatrix(numberone(), 0);
...
//	printMat(m_C0, "m_C0");
    ...
    for(int it = 0; it < m_niter; it++) {
		...
		//	cout << "chisq " << it << " : " << m_chi << endl;
		//	cout << "delta chisq " << it << " : " << delchi << endl;
	...
	m_myniter = iter_tmp;
	m_flag_break = flag_break;
//	cout << "flag_break : " << flag_break << endl;
    if(!flag_break) {
        return okfit;
    }
                                 ...
                                 switch(ptype) {
                                     case 0 :{
										...
										//  cout << "FourMomentum" << endl;
										  //printMat(ta, "m_ta FourMomentum");
										//  printMat(m_A, "m_A FourMomentum");
                                                 break;
                                             }
                                     case 1 :  {
                                                   HepMatrix ta(1, NTRKPAR, 0);
