...
p_high = 1.2
cos_theta_range = np.arange(0, 0.9+1e-6, 0.05).round(3)
...
p_range = np.arange(p_low, p_high+(1e-6), 0.05).round(3)
...
if 'nbar' in args.particle_type:
    ...
    df = readRootFiles(glob(os.path.join(args.path, '*.root'))[:1], particle=args.particle_type)
